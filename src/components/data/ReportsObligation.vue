<template>
    <div>
        ...
        <div class="container" id="print" ref="content">
            <div class="text-center text-uppercase fw-bold">{{ details.recur }} Obligation</div>
            ...
            <div class="row ps-4 bordered bg-mint">
                <div class="col-6 fw-bold d-flex justify-content-between pe-4">
                    <span>Grand Total</span>
                    <span> &#8369; {{ formatNumber(grandtotal.toFixed(2)) }}</span>
                </div>
                <div class="col-2 fw-bold br-left text-end">
                    {{ formatNumber(totalps.toFixed(2)) }}
                </div>
                <div class="col-2 fw-bold br-left text-end">
                    {{ formatNumber(totalmooe.toFixed(2)) }}
                </div>
                <div class="col-2 fw-bold br-left text-end">
                    {{ formatNumber(totalco.toFixed(2)) }}
                </div>
            </div>
            <div v-for="item in subtotals" :key="item" class="row ps-4 bordered">
                ...
                <div class="col-2 font-weight-bold br-bottom br-left bg-lmint text-end">
                    {{ formatNumber(item.totalps.toFixed(2)) }}
                </div>
                <div class="col-2 font-weight-bold br-bottom br-left bg-lmint text-end">
                    {{ formatNumber(item.totalmooe.toFixed(2)) }}
                </div>
                <div class="col-2 font-weight-bold br-bottom br-left bg-lmint text-end">
                    {{ formatNumber(item.totalco.toFixed(2)) }}
                </div>

                <div v-for="data in finalSorted" :key="data.id" class="br-right"> 
                    <div v-if="item.pap === data.pap" class="row">
                        <div class="col-6 ps-4 br-bottom br-left">
                            {{ data.uacscode }}
                        </div>
                        <div class="col-2 br-bottom br-left text-end">
                            {{ data.allotclass === 'PS' ?  formatNumber(data.obligation.toFixed(2)) : ''}}
                        </div>
                        <div class="col-2 br-bottom br-left text-end">
                            {{ data.allotclass === 'MOOE' ?  formatNumber(data.obligation.toFixed(2)) : ''}}
                        </div>
                        <div class="col-2 br-bottom br-left text-end">
                            {{ data.allotclass === 'CO' ?  formatNumber(data.obligation.toFixed(2)) : ''}}
                        </div>
                    </div>
                </div>
            </div>
...
export default {
        ...
        totalps() {
            ...
            for (let index = 0; index < this.sortedArray.length; index++) {
                if(this.sortedArray[index].allotclass === 'PS'){
                    total = total + this.sortedArray[index].obligation
                }           
            }
            return total
        },
        ...
        sortDataByCode(){
            ...
            this.filteredData.forEach(index => {
                ...
                if(!data){
                    ...
                    data.obligation = parseFloat(data.obligation) + parseFloat(index.obligation)
                }
            })
            return holder
        }
    ...
    methods: {
        ...
        print() {
            ...
            filename:     `RAOD_Obligation_${ this.recurr }`,
